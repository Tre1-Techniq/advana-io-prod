...
const initialFormState = {
 firstname: '', lastname:'', company: '', segment: '', email: '', password: '', conformPassword: '', formType: 'signUp'
}
...
const userAuth = (props) => {
  ...
  const onSignUpSubmit = async (event) => {
    ...
    //const res = await axios.post(`${config.api.invokeUrl}/users/${id}`, params);
  ...
  const onSignInSubmit = (event, data) => {
    event.preventDefault();
  ...
  const onConfirmSignUpSubmit = event => {
    event.preventDefault();
    ...
    user.confirmRegistration(code, true, (err, result) => {
      ...
      try {
        ...
        console.error('Your confirmation is incorrect', err);
        return;
      }
    });
