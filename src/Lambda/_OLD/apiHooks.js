...
const dynamodbTableName = 'UserData';
...
exports.handler = async function(event) {
  ...
  switch(true) {
    ...
    case event.httpMethod === 'PATCH' && event.path === userPath:
      ...
      response = await modifyUser(requestBody.email, requestBody.updateKey, requestBody.updateValue);
      break;
    case event.httpMethod === 'DELETE' && event.path === userPath:
      response = await deleteProduct(JSON.parse(event.body).email);
      break;
    default:
...
async function modifyUser(email, updateKey, updateValue) {
  const params = {
    TableName: dynamodbTableName,
    Key: {
      'email': email
